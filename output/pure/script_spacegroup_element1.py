...
usage    = ' Usage: %s [element], lattice constant,  h,k,l \n ' % sys.argv[0]
...
if len(sys.argv) == 6:
	element  = sys.argv[1]
	a0       = float(sys.argv[2])
	MI       = (int(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5]))
...
mpr = MPRester()
fcc_metal = Structure.from_spacegroup("Fm-3m", Lattice.cubic(a0), [element, element], [[0, 0, 0], [0.5, 0.5, 0.5]])
...
plt.savefig('slab_%s_site.png' % element)
...
while (i < len(ads_structs)):
	...
	plt.savefig('slab_%s_absorb%i.png' % (element,i))
	i+=1
#plt.show()
...
os.system('mv temp %s_%i%i%i' % (element,MI[0],MI[1],MI[2]))
